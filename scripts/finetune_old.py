...
# def _mlflow_log_metrics_renamed(
...
#         else:
#             mlflow.log_metrics(renamed, step=step)


# class StepMetricLogger(TrainerCallback):
#     """Log metrics every ``log_frequency`` training steps to the python logger and MLflow."""

#     def __init__(self, log_frequency: int, enable_mlflow: bool) -> None:
...
#         step = state.global_step  # type: ignore[attr-defined]
#         if step is None or step <= 0 or step % self.log_frequency != 0:
#             return
...
#         LOGGER.info("Step %s metrics: %s", step, numeric_logs)

#         if self.enable_mlflow and mlflow.active_run():
#             _mlflow_log_metrics_renamed(numeric_logs, step=step)
...
def data_processing(cfgs, tokenizer, split_name: str = "training"):
    ...
    tokenize_fn = build_tokenize_fn(
        tokenizer, cfgs["prompt"]["template"], int(cfgs["data"].get("max_length", 1024))
    )
...
def build_metrics_computer(tokenizer):
    ...
    def compute_metrics(eval_pred):
        ...
        if valid_idx:
            ...
            macro_f1 = f1_score(
                vt, vp, labels=sorted(LABELS), average="macro", zero_division=0
            )
...
def train(model, tokenizer, cfgs: dict[str, Any], training_runtime: dict[str, Any], ):
    ...
    save_steps = training_runtime["save_steps"]
...
def post_eval(
    ...
    if labels is not None:
        for pred_row, label_row in zip(preds, labels, strict=False):
            ...
            text = (
                tokenizer.decode(target_ids, skip_special_tokens=True).strip().lower()
            )
            results.append(text)
    else:
    ...
    statements = (
        list(eval_split["statement"]) if "statement" in eval_split.column_names else []
    )
    verdicts = (
        list(eval_split["verdict"]) if "verdict" in eval_split.column_names else []
    )
    ...
    if statements and verdicts:
        ...
        target_dir = Path(output_dir).resolve() if output_dir else Path(
            "results/ar-qwen-mini"
        ).resolve()
